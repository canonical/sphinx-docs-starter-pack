...
# e.g. user@device:~/git/Canonical/sphinx-docs-starter-pack/docs$ DEBUGGING=1 python _utils/update_sp.py
...
GITHUB_API_UTILS_DIR = f"{GITHUB_API_BASE}/contents/docs/_utils"
...
def main():
    ...
    if parse_version(local_version) < parse_version(latest_release):
        ...
        # Identify and download '_utils' dir files to '_utils/update'
        files_updated, new_files = update_static_files()

        # Write new version to file to '_utils/update'

        download_file(
            GITHUB_RAW_BASE + "/docs/_utils/version",
        ...
        if files_updated:
            ...
            print("Updated files have been downloaded to '_utils/update'.")
            print("Validate and move these files into your '_utils/' directory.")
        else:
        ...
        if new_files:
            ...
            print(
                ...
                "Validate and merge these files into your '_utils/' directory",
            )
        else:
    ...
    try:
        ...
        print(
            "The updated starter pack has moved requirements.txt out of the '_utils' dir"
        )
        print("requirements.txt not checked, please update your requirements manually")


def update_static_files():
    """Checks local files against remote for new and different files, downloads to '_utils/updates'"""
    files, paths = get_local_files_and_paths()
    new_file_list = []

    for item in query_api(GITHUB_API_UTILS_DIR).json():
        logging.debug(f"Checking {item['name']}")
        # Checks existing files in '_utils' starter pack static root for changed SHA
        if item["name"] in files and item["type"] == "file":
            ...
            if item["sha"] != get_git_revision_hash(paths[index]):
                ...
                if item["name"] == "update_sp.py":
                    ...
                    print("WARNING")
                    print(
                        "THIS UPDATE SCRIPT IS OUT OF DATE. YOU MAY NEED TO RUN ANOTHER UPDATE AFTER UPDATING TO THE FILE IN '_utils/updates'."
                    )
                    print("WARNING\n")
            else:
                logging.debug("File hashes are equal")
        # Checks nested files '_utils/**/**.*' for changed SHA (single level of depth)
        elif item["type"] == "dir":
            logging.debug(item["name"] + " is a directory")
            for nested_item in query_api(
                f"{GITHUB_API_UTILS_DIR}/{item['name']}"
            ).json():
        ...
        # Downloads NEW files in '_utils' starter pack static root
        else:
            if item["type"] == "file":
...
def get_local_files_and_paths():
    """Identify '_utils' local files and paths"""
